defmodule HiveforgeController.JobService do
  alias HiveforgeController.{Job, Repo, ApiKeyService}
  import Ecto.Query

  def create_job(attrs, claims) do
    case ApiKeyService.authorize_action(claims, :create_job) do
      :ok ->
        %Job{}
        |> Job.changeset(attrs)
        |> Repo.insert()
  ...
  def get_job(id, claims) do
    case ApiKeyService.authorize_action(claims, :get_job) do
      :ok ->
  ...
  def list_jobs(claims) do
    case ApiKeyService.authorize_action(claims, :list_jobs) do
      :ok ->
        jobs =
          Job
